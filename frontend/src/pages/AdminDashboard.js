...
import { ArrowLeft, RefreshCw, Upload, Package, CheckCircle, Clock, XCircle, Search, Bell, Trash2, Eye, Plus, User as UserIcon, Settings, FolderOpen } from "lucide-react";
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="space-y-1.5">
                      ...
                      >
                        ...
                        <SelectContent>
                          <SelectItem value="clay">Stage: {getStageLabel('clay', workflowConfig)}</SelectItem>
                          <SelectItem value="paint">Stage: {getStageLabel('paint', workflowConfig)}</SelectItem>
                          <SelectItem value="fulfilled">Stage: {getStageLabel('shipped', workflowConfig)}</SelectItem>
                  ...
                  <div className="grid grid-cols-1 xl:grid-cols-2 gap-3">
                    ...
                    <div className="relative p-3 bg-yellow-50 rounded-lg border border-yellow-200 min-w-0">
                      <Badge className={`absolute top-2 right-2 text-white text-xs py-0 uppercase ${
                        order.clay_status === 'approved' ? 'bg-green-500' :
                        order.clay_status === 'changes_requested' ? 'bg-orange-500' :
                        order.clay_status === 'feedback_needed' ? 'bg-blue-500' :
                        order.clay_status === 'sculpting' ? 'bg-gray-500' : 'bg-gray-400'
                      }`}>{getStatusLabel(order.clay_status, workflowConfig)}</Badge>
